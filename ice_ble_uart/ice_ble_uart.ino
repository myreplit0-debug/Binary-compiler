...
static size_t cobs_decode(const uint8_t* in, size_t len, uint8_t* out, size_t outMax) {
  if (!len) return 0;
  const uint8_t* ip = in; const uint8_t* end = in + len; uint8_t* op = out;
  while (ip < end) {
    uint8_t code = *ip++;
...
static void loadBinds(){
  mac2dev.clear();
  File f=SPIFFS.open("/binds.csv", FILE_READ); if(!f) return;
  while (f.available()){
    String line=f.readStringUntil('\n'); line.trim(); if(!line.length()) continue;
    int c=line.indexOf(','); if(c<0) continue;
    String smac=line.substring(0,c), sn=line.substring(c+1);
    uint8_t m[6]; if(!parseMac(smac,m)) continue; MacKey k; memcpy(k.b,m,6);
    mac2dev[k]=(uint16_t)sn.toInt();
  }
  f.close();
}
...
static void loadNames(){
  devNames.clear();
  File f=SPIFFS.open("/names.csv", FILE_READ); if(!f) return;
  while (f.available()){
    String line=f.readStringUntil('\n'); line.trim(); if(!line.length()) continue;
    int c=line.indexOf(','); if(c<0) continue;
    uint16_t id=(uint16_t)line.substring(0,c).toInt(); String nm=line.substring(c+1);
    devNames[id]=nm;
  }
  f.close();
}
...
static void pollUsbCommands(){
  while (Serial.available()){
    ...
    if (c=='\r' || c=='\n'){ inLine.trim(); if(inLine.length()) handleCommand(inLine); inLine=""; }
    else inLine += c;
  }
}
...
static void pollICE(){
  while (Serial1.available()){
    ...
    if (b==0x00){
      ...
      else frameLen=0; // overflow guard
    }
  }
}
...
static void endOfScanFlush(){
  for (auto &kv : bestRssi){
    uint16_t dev=kv.first; int8_t rssi=kv.second;
