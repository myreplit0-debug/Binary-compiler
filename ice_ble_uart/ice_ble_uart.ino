...
void processDecodedMessage(const uint8_t* m, size_t n, uint8_t src_room, bool /*from_esn*/){
  ...
  if(given!=calc){ Serial.printf("[PARSE] CRC FAIL len=%u given=0x%04X calc=0x%04X\n",(unsigned)n,(unsigned)given,(unsigned)calc); return; }
  ...
  if(typ==MT_END){
    ...
    for(uint16_t i=0; i<count && p+2<=end; ++i){
      ...
      if(!(T==1 && L==6) || p+6>end){ Serial.printf("[PARSE] rec%u bad T1/L1 T=%u L=%u\n",(unsigned)i,(unsigned)T,(unsigned)L); break; }
