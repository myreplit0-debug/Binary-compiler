/*
  SMOKE Unified (ESP32-S3) — Receiver/Bridge v2.2 (room-aware, name debug)
  - Designed for esp32 core 3.x
  - UART1 (GPIO16 RX, GPIO17 TX) COBS+CRC input from ICE
  - Name/MAC binds, sanitized ASCII prefix for names
  - Per-room best-RSSI dedupe, ESP-NOW relay with frag/reasm
  - Tail prints compact summary:  room.device@rssi
  - NEW: runtime debugs:
      * "dbg parse on|off"  -> [PARSE] flow logs
      * "dbg names on|off"  -> hex dump RAW TLV-4 name + SAN result
*/
...
#define ESN_FRAG_DATA_MAX 200     // keep headroom for header (payload max ~250)
...
struct ReasmBuf {
  ...
  std::vector<uint8_t>  data;         // slot-per-fragment
  uint32_t last_ms;
};
...
static bool     g_dbg_parse=true;     // [PARSE] logs
static bool     g_dbg_names=false;    // RAW name hex + SAN print
...
static String sanitizePrefix(const String& in){
  ...
  out.trim();
  String col; col.reserve(out.length()); bool sp=false;
  for (char c:out){ if(c==' '||c=='\t'){ if(!sp){ col+=' '; sp=true; } } else { col+=c; sp=false; } }
  return col;
}
static uint16_t crc16_ccitt(const uint8_t* d,size_t n,uint16_t crc=0xFFFF){
  while(n--){ crc^=(uint16_t)(*d++)<<8; for(int i=0;i<8;i++) crc=(crc&0x8000)?((crc<<1)^0x1021):(crc<<1); }
  return crc;
}
...
static void loadEsn(){ File f=SPIFFS.open("/esn.txt", FILE_READ); if(f){ String line=f.readStringUntil('\n'); f.close(); int c=line.indexOf(','); if(c>0){ g_esn_on=line.substring(0,c).toInt()!=0; uint16_t ch=line.substring(c+1).toInt(); if(ch>=1&&ch<=13) g_channel=(uint8_t)ch; } } }
...
static void printHelp(){
  ...
  Serial.println(F("  dbg parse on|off         ; toggle parse logs"));
  Serial.println(F("  dbg names on|off         ; dump RAW name bytes + SAN"));
...
static void handleCommand(const String& s){
  ...
  if(cmd=="help"||cmd=="?"){ printHelp(); }
    ...
    int idx=rest.substring(0,sp2).toInt(); uint16_t id=(uint16_t)rest.substring(sp2+1).toInt();
    if(idx<0||(size_t)idx>=seenNames.size()){ Serial.println(F("ERR: idx out of range")); return; }
    String san=seenNames[(size_t)idx]; name2dev[san]=id; Serial.printf("OK bindseen [%d] \"%s\" -> %u\n", idx, san.c_str(), (unsigned)id);
  }
  else if(cmd=="show"){
    ...
    String what, val; int sp2=rest.indexOf(' '); if(sp2>0){ what=rest.substring(0,sp2); val=rest.substring(sp2+1); what.toLowerCase(); val.toLowerCase(); }
    bool *flag=nullptr;
    if(what=="parse") flag=&g_dbg_parse;
    if(what=="names") flag=&g_dbg_names;
    if(!flag){ Serial.println(F("ERR: dbg parse|names on|off")); return; }
    if(val=="on"||val=="1"||val=="true") *flag=true;
    else if(val=="off"||val=="0"||val=="false") *flag=false;
    else { Serial.println(F("ERR: use on|off")); return; }
    Serial.printf("OK dbg %s=%d\n", what.c_str(), (int)*flag);
  }
...
static bool esn_setup(){
  ...
  esp_now_peer_info_t peer{}; memcpy(peer.peer_addr, ESN_BROADCAST, 6); peer.channel=g_channel; peer.encrypt=false;
  esp_now_add_peer(&peer);

  // recv callback (esp32 3.x signature)
  esp_now_register_recv_cb([](const esp_now_recv_info_t* info, const uint8_t* data, int len){
    ...
    if(complete){
      ...
      if(g_dbg_parse) Serial.printf("[ESN-RX] complete src_room=%u bytes=%u%s\n", (unsigned)rb.src_room, (unsigned)msg.size(), duplicate?" (duplicate)":"");
      if(!duplicate) processDecodedMessage(msg.data(), msg.size(), rb.src_room, true);
      reasmList.erase(reasmList.begin()+slot);
    }
  });

  // use simple (old-style) send-cb signature for core 3.x; or skip entirely
...
static void forward_upstream(uint8_t src_room, const uint8_t* decoded, size_t n,
...
void processDecodedMessage(const uint8_t* m, size_t n, uint8_t src_room, bool /*from_esn*/){
  ...
  if(given!=calc){ if(g_dbg_parse) Serial.printf("[PARSE] CRC FAIL len=%u\n",(unsigned)n); return; }
  ...
  if(g_dbg_parse) Serial.printf("[PARSE] OK typ=%u src_room=%u batch=%lu seq=%u\n",(unsigned)typ,(unsigned)src_room,(unsigned long)batch,(unsigned)seq);
  ...
  if(typ==MT_END){
    ...
    if(g_dbg_parse) Serial.printf("[PARSE] count=%u\n",(unsigned)count);
    ...
    for(uint16_t i=0; i<count && p+2<=end; ++i){
      ...
      if(p+2<=end && m[p]==4){
        T=m[p++]; L=m[p++]; if(p+L<=end){
          nameRaw = String((const char*)&m[p], L);
          if (g_dbg_names){
            Serial.printf("[NAME] src_room=%u len=%u raw=", (unsigned)src_room, (unsigned)L);
            hexDump(&m[p], L); Serial.println();
          }
          p+=L;
        }
      } else if (g_dbg_names){
        Serial.println("[NAME] (no T=4 name in this record)");
      }

      if (nameRaw.length()){
        nameSan = sanitizePrefix(nameRaw);
        if (g_dbg_names){
          Serial.printf("[NAME] SAN=\"%s\"\n", nameSan.c_str());
        }
        addSeen(nameSan);
      }
      ...
      if (g_dbg_parse){
        ...
        Serial.printf("[REC] mac=%s rssi=%d %s -> %s\n",
          macStr,(int)rssi,
          nameSan.length()? String("SAN=\""+nameSan+"\"").c_str() : "(no name)",
  ...
  while(Serial1.available()){
    ...
    if(b==0x00){
      if(frameLen>0){
        ...
        if(dec>0) processDecodedMessage(decodeBuf, dec, g_room, false);
        frameLen=0;
      }
    } else {
...
void setup(){
  ...
  Serial1.setRxBufferSize(ICE_UART_RXBUF); Serial1.setTxBufferSize(ICE_UART_TXBUF);
