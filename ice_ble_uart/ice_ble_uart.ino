/*
  ...
  --------------------
  - Room 1 (tail=off): prints EXACTLY what it will send over ESP-NOW
      [ESN-TX RAW] ... + hex dump + parsed TLVs + sanitize results
  - Room 2 (tail=on): prints EXACTLY what it reassembled from ESP-NOW
      [ESN-RX RAW] ... + hex dump + parsed TLVs + sanitize results
  ...
  Optional debug filters:
    dbg hex on       ; show hex dumps
    dbg parse on     ; show parse details / sanitize results
    dbg names on     ; only prints name-related lines when parsing
*/
...
// Debug toggles
static bool DBG_HEX   = true;    // hex dumps of TX/RX buffers
static bool DBG_PARSE = true;    // structured parse logs
static bool DBG_NAMES = true;    // print name/sanitize/binding lines
...
// --------- room-aware "best RSSI this scan" (for tail printers) ---------
struct RoomDevKey {
  uint8_t  room;
  uint16_t dev;
...
struct ReasmBuf {
  ...
  std::vector<uint8_t> data;   // flat buffer (chunked by ESN_FRAG_DATA_MAX)
  uint32_t last_ms;
};
...
static bool esn_setup(){
  ...
  esp_now_register_recv_cb([](const esp_now_recv_info_t* info, const uint8_t* data, int len){
    ...
    // Accept: only src_room == myRoom or myRoom-1
  ...
  // SEND (3.x API signature)
  esp_now_register_send_cb([](const uint8_t* mac, esp_now_send_status_t status){
    (void)mac;
    if (!DBG_PARSE) return;
    Serial.printf("[ESN-SEND] status=%s\n", status==ESP_NOW_SEND_SUCCESS?"OK":"FAIL");
  });
...
static void parseAndMaybeRecord(const uint8_t* m, size_t n, uint8_t src_room){
  // assume header+CRC validated by caller
  ...
  for (uint16_t i=0; i<count && p+2<=end; ++i){
    ...
    if (DBG_PARSE && (!DBG_NAMES || nameRaw.length())){
      Serial.printf("[REC] room=%u mac=%s rssi=%d RAW=\"%s\" SAN=\"%s\" -> %s\n",
                    (unsigned)src_room, macStr, (int)rssi,
                    nameRaw.c_str(), nameSan.c_str(),
...
void processDecodedMessage(const uint8_t* m, size_t n, uint8_t src_room, bool /*from_esn*/){
  ...
  // de-dup by (origin,batch,seq,src_room)
  ...
  for (auto &k : seenCache){
    if (k.src_room==sk.src_room && k.batch==sk.batch && k.seq==sk.seq && memcmp(k.origin,sk.origin,6)==0) goto maybe_forward;
  }
  if (seenCache.size() >= SEEN_CACHE_MAX) seenCache.erase(seenCache.begin());
  seenCache.push_back(sk);
  ...
  if (typ == MT_END){
    ...
    parseAndMaybeRecord(m, n, src_room);
  }
  ...
  while(Serial1.available()){
    ...
    if (b==0x00){
      if (frameLen>0){
        ...
        if (dec>0) {
          // If this device is Room 1 (tail off), this is the *source* that will forward.
          // Regardless of role, print what came from ICE so we can compare against ESN-TX RAW.
      ...
      if (frameLen<FRAME_MAX) frameBuf[FRAME_MAX>0?frameLen++:frameLen]=b; else frameLen=0;
    }
  }
}
